...
U-Rest's core promise is escrow: guest pays 100% at booking, U-Rest holds the money, host is paid after the guest checks out. The gateway must therefore support (a) PromptPay + cards for Thai consumers, (b) holding funds between charge and payout, (c) paying out to third-party (host) bank accounts, and (d) onboarding **before** the company is registered — the team launches as an individual pilot (ADR-scope: see BUSINESS_PLAN.md §Legal) on a ฿1,000/month budget.
...
3. **Escrow = Opn balance + our Postgres ledger** (ADR-003), not a gateway feature. Funds stay in the Opn balance; payouts run after checkout (booking COMPLETED, ADR-012) — releasing at checkout rather than mid-stay also sits more comfortably inside Opn's 7-day transferable-hold window.
