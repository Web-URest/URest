...
**The product:** every villa is identity-verified before listing (Thai ID + right-to-rent documents, reviewed by a human); guests pay 100% through the platform (PromptPay/card via Opn Payments); **U-Rest holds the money in escrow and releases it to the host only 24 hours after successful check-in**. Disputes freeze the payout. Contact details are masked until payment, so the scam can't move off-platform. An AI concierge (น้องเรสต์, built on Claude) helps groups find and compare villas — but only a human ever pays.
...
Verified listings → request-to-book or instant-book → 100% in-app payment → escrow ledger (HELD → RELEASABLE → PAID, FROZEN on dispute) → host paid T+24h after check-in. In-app messaging with contact masking until confirmed. Per-villa calendars, seasonal pricing with Thai-holiday awareness. AI concierge for discovery with a hard no-payment boundary. Admin = verify humans, move money, judge disputes; everything else automated.
...
**Positioning (locked):** the trust layer versus FB-direct. Marketing names the scam, not competitors — "จองพูลวิลล่าไม่ต้องเสี่ยงโดนโกง: เงินถึงเจ้าของหลังเช็คอินเท่านั้น." Broker directories are treated as a host-recruitment pool, never attacked. The verification gate is framed as the brand: "ทุกที่พักผ่านการตรวจสอบ เพื่อให้ผู้เข้าพักกล้าจ่าย."
...
- *What about leakage (guest rebooks direct next year)?* **Accepted.** Pre-payment poaching is blocked (contact masking — that's where scams happen); post-stay direct rebooking is unenforceable and policing it would poison host relations. U-Rest sells *new-guest acquisition*; retention comes from product gravity (reviews, ranking, tools), and a repeat-discount mechanic is parked for v2.
...
| Organizer won't pay 100% upfront to an unknown brand | Medium | Escrow messaging at every step, card option (chargeback comfort), refund promise ≤24h dispute window, university seeding builds first reviews among forgiving users |
