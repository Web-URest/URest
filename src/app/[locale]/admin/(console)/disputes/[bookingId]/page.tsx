...
export default async function DisputeCasePage({ params }: { params: Promise<{ bookingId: string }> }) {
  ...
  return (
    <section className="flex max-w-2xl flex-col gap-6">
      <div>
        <Link href="/admin/disputes" className="text-sm text-sand-400 hover:text-sand-200">
          {t("backToQueue")}
        </Link>
        <h1 className="mt-2 text-2xl font-bold">{dispute.booking.code ?? bookingId}</h1>
        <p className="mt-1 text-sand-300">{dispute.booking.listing.title}</p>
      ...
      <div className="flex flex-col gap-4 border-t border-ink-700 pt-6">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-sand-300">{t("evidence")}</h2>
        {reports.length === 0 ? (
          <p className="text-sand-400">{t("noReports")}</p>
        ) : (
          reports.map((r, i) => (
            <div key={r.id} className="flex flex-col gap-2">
              ...
              <p className="whitespace-pre-wrap text-sand-200">{r.text}</p>
      ...
      <div className="flex flex-col gap-3 border-t border-ink-700 pt-6">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-sand-300">{t("chatEvidence")}</h2>
        <p className="rounded bg-ink-800 p-2 text-xs text-amber-400">{t("pdpaNotice")}</p>
        {thread.messages.length === 0 ? (
          <p className="text-sand-400">{t("noMessages")}</p>
        ) : (
          <ul className="flex flex-col gap-2">
            {thread.messages.map((m) => (
              <li key={m.id} className="text-sm">
                ...
                <p className="text-sand-100">{m.body}</p>
              </li>
            ))}
          </ul>
        )}
      ...
      {isOpen || awaitingAppeal ? (
        <form action={action.bind(null, bookingId)} className="flex flex-col gap-4 border-t border-ink-700 pt-6">
          ...
          <fieldset className="flex flex-col gap-2">
            ...
            <label className="flex items-center gap-2 text-sm text-sand-200">
              ...
              <input
                ...
                max={100}
                defaultValue={50}
                className="ml-2 w-20 rounded border border-ink-700 bg-ink-800 px-2 py-1 text-sm text-sand-100"
              />
