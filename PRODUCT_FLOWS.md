...
**Home (`home.html`)** — search card (region/dates/guests), AI concierge entry banner, trust row, region rail, featured villas. The trust row exists because first-time visitors arrive skeptical (they've been burned on Facebook); the page must answer "why is this safe?" before "what's available?".

**Search results (`search.html`)** — filter chips (price, capacity, สระส่วนตัว, คาราโอเกะ, BBQ, สัตว์เลี้ยงได้, สไลเดอร์…), sort (แนะนำ / ราคา / เรตติ้ง), desktop map with price pins, mobile map toggle. Only PUBLISHED listings with availability for the chosen dates appear — never show what can't be booked.

**AI concierge "น้องเรสต์" (`concierge.html`)** — chat that uses backend tools only:
...
### 4.2 Host dashboard (`host.html`) — tab by tab
...
### 4.4 Edit Villa page (`host-edit.html`)
...
- Every mockup page maps to a flow: home/search/listing → §3.1, booking → §3.2, trips → §3.3, concierge → §3.1, host → §4.2, host-wizard → §4.1, host-edit → §4.4, admin → §5. The messaging UI (§3.5) has **no mockup yet** — add one before Phase 3 build.
