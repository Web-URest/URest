...
|---|---|---|
| **Guest** | Default on signup — email+password, Google, Facebook, or LINE (ADR-007) | Search, AI concierge, booking, trips, messages, reviews |
...
- **ถูกต้องตามกฎหมาย badge (host, optional)** → host additionally uploads a hotel license or non-hotel registration (สถานที่พักที่ไม่เป็นโรงแรม, the ≤8-room exemption under the Hotel Act). Earns a visible badge on the listing + a search-ranking boost (decision 2026-06-12). Never required for approval — required tier is identity/right-to-rent; the licensing tier is encouraged and rewarded. Host T&Cs make the host warrant Hotel Act compliance **and that bookings for the dates they list are taken only through U-Rest (in-app-only obligation, ADR-012)**; U-Rest acts as intermediary.
...
stateDiagram-v2
    ...
    AWAITING_PAYMENT --> CONFIRMED : payment received (≤12h request / ≤1h instant)
    AWAITING_PAYMENT --> EXPIRED : no payment (12h request / 1h instant)
    ...
    CHECKED_IN --> DISPUTED : guest reports issue during stay (until checkout)
...
| CONFIRMED → CHECKED_IN | Automatic | at check-in date 15:00 | Issue-report window opens (runs through the stay, until checkout). |
| CHECKED_IN → COMPLETED | Automatic | at checkout date 11:00 | Payout → RELEASABLE (no open dispute). Review window opens (14 days, both directions). |
| CHECKED_IN → DISPUTED | Guest taps "แจ้งปัญหาที่พัก" | any time during the stay (until checkout) | Payout frozen. Admin case opened with booking's message thread + photos as evidence. |
...
`HELD` (from payment) → `RELEASABLE` (at checkout / COMPLETED, no open dispute) → `PAID` (admin transfers, §5.2) · `FROZEN` (dispute opened) · `REVERSED` (refunded to guest)
...
flowchart TD
    ...
    LED -->|"checkout (COMPLETED), no open dispute"| REL[RELEASABLE]
...
3. **ชำระเงิน** — coral countdown banner (12h), PromptPay QR as the **default tab** (15-min validity, auto-confirm via webhook, regenerate button) with card form behind a second tab (fee-steering, §2.3), full escrow strip, "ไม่ตรงตามประกาศ แจ้งก่อนเช็คเอาท์ คืนเงินเต็มจำนวน" promise.
...
| CHECKED_IN | เช็คอินแล้ว | แจ้งปัญหาที่พัก (ก่อนเช็คเอาท์) | message host |
| DISPUTED | มีข้อพิพาท (⚠) | ดูสถานะข้อพิพาท | message host |
| COMPLETED | เข้าพักแล้ว | ⭐ เขียนรีวิว | จองซ้ำ |
| DECLINED/EXPIRED | greyed | หาที่คล้ายกัน | — |
| CANCELLED_* | ยกเลิกแล้ว (greyed) **+** money pill คืนเงินแล้ว/REFUNDED when payout `REVERSED` | refund status | — |
...
- **Pre-CONFIRMED masking**: phone numbers, LINE IDs, URLs and bank account numbers are auto-redacted from messages until payment. Why: the entire scam pattern starts with "โอนมาทางนี้เลย ถูกกว่า" — booking and paying outside U-Rest is prohibited (host obligation, ADR-012), and pre-payment masking is the technical floor that enforces it before money can move.
...
Login via email+password, Google, Facebook, or LINE (ADR-007; accounts linked by verified email); **"เชื่อมต่อ LINE" to enable LINE push** if signed in another way; profile (name, photo, phone); saved villas (♡ → `/saved`, §3.1); notification preferences (LINE/email per event group); PDPA: consent at signup, data export and delete-account request (delete is soft + anonymized if bookings exist — ledger integrity).
...
|---|---|---|
| **แจ้งปัญหาที่พัก** (dispute) | any time during the stay (until checkout) | Payout auto-FROZEN; full-refund promise applies (§2.1) |
...
| **ปฏิทิน** | **One calendar per villa** — villa switcher chips at top, never a merged view; per-villa 2-month grid: ว่าง / จองผ่าน U-Rest (with guest name) / ปิดเอง (diagonal stripes); tap-drag to block | Double-booking prevention. **New bookings for listed dates must run through U-Rest (host obligation, ADR-012)**; the ปิดเอง block is for legitimate unavailability — owner use, maintenance, or commitments predating onboarding — and must be kept current so a paid U-Rest guest is never double-booked. The banner warns that double-booking a paid U-Rest guest forces CANCELLED_BY_HOST + strike (3 strikes = suspension). Merged multi-villa views cause block-the-wrong-villa mistakes — that's why each villa's calendar stands alone. Extra urgency for ⚡ instant listings: there is no accept step to catch a stale calendar. |
...
- "Due" list = bookings with payout RELEASABLE (at checkout / COMPLETED, no dispute, no hold), grouped by host bank account.
...
- Sources: guest "แจ้งปัญหาที่พัก" (any time during the stay, until checkout) or admin-initiated (e.g., host reports guest damage — v1: recorded but deposit is cash, so mostly advisory).
...
- All timers: host accept **12h**, guest pay **12h** (request) / **1h** (instant), QR validity **15 min**, dispute window **check-in → checkout** (the whole stay), payout release **at checkout (COMPLETED)**, review window **14 days**, admin queue SLA **24h**, report triage SLA **24h**.
...
- Every screen in the design system (`src/components/ui/` + the page routes, catalogued at `/styleguide`) maps to a flow: home/search/listing/saved → §3.1, booking → §3.2, trips → §3.3, messages → §3.5, concierge → §3.1, host dashboard/wizard/calendar/earnings → §4, admin console → §5. Design-vs-flows gaps are tracked in DESIGN_SPEC §9 (historical build checklist) — the audited admin-as-login-role (§9 A2) must NOT be copied.
